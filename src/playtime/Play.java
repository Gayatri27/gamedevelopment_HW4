...
import model.Player;
import model.Prey;
import networking.response.ResponseSDReconnect;
...
//    private 
    ...
    public void addPlayer(Player player) throws IOException{
        ...
        try{
            ...
            PlayTimePlayer ptp = rPlayers.get(player.getID()); 
            Log.printf_e("reconnecting player %d to game %d", ptp.getID(), playID);
            ...
            this.rPlayers.put(player.getID(), new PlayTimePlayer(player.getID(), playID));
        
            try {
                PlayDAO.createPlayer(player.getID(), playID,0);
            } catch (SQLException e) {
                Log.println_e("Error in writing record of player ID " + player.getID()+ " in play ID " + playID + " into database.");
                Log.println_e(e.getMessage());
            }
        }
    }
    ...
    public Play(List<Player> players, int playID) {
        ...
        for (Player player : players) {
            ...
            try
            {
                PlayDAO.createPlayer(player.getID(), playID,i);
                i++;
            }
            catch (SQLException e)
            {
                Log.println_e("Error in writing record of player ID " + player.getID()+ " in race ID " + playID + " into database.");
                Log.println_e(e.getMessage());
            }
